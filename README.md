...
Google Sheets API 遇到 `429/500/502/503/504`、网络超时或连接重置时，会按 `1/2/4` 秒自动重试；多次重试仍失败时只返回简短提示，不会把 Google HTML 错误页发送到群里。
...
机器人会先在后台“用户ip记录”中按“客户端ip地址”查询用户唯一 ID，再到“用户列表”按用户 ID 查询，回复对应的 WPPChat 号和注册时间。
